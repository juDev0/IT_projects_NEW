...
class AssignmentTracker(QWidget):
    def __init__(self):
        ...
        self.init_ui()
        self.assignmets = []  # this holds all the assingmetn reason: so it can be sorted
    ...
    def add_assignment(self):
        ...
        if title.strip():
            ...
            self.assignmets.append({
                "title": title,
                "due": due_date
            })

            # Sort the list by due date
            self.assignmets.sort(key=lambda x: x["due"])
    ...
    def save_assignments(self):
        ...
        with open("assignmets.json", "w") as file:
            json.dump(data, file, indent=2)


    # loads
    def load_assignments(self):
        if os.path.exists("assignmets.json"):
            with open("assignmets.json", "r") as file:
                data = json.load(file)

            self.assignmets.clear()
    ...
    def delete_assignment(self):
        ...
        if selected_row >= 0:
            ...
            if reply == QMessageBox.Yes:
                del self.assignmets[selected_row]  # delets the item in the self.ass..list
